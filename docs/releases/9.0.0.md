...
- [Release notes graphql-compose-mongoose v9.0.0](#release-notes-graphql-compose-mongoose-v900)
...
In such a case, you will get the error in the `userCreate.error` field and the top-level `errors` field will be undefined:
...
Before 9.0.0, `graphql-compose-mongoose` would save some records provided to `createMany` even if another had failed with a validation error. Now it will first check that all records are valid before save; and if some records contain errors, then no one document will be saved.
...
Special thanks to our sponsors which have joined recently:
