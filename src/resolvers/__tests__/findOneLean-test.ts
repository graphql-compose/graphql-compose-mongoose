...
describe('findOneLean() ->', () => {
  it('should return Resolver object', () => {
    const resolver = findOneLean(UserModel, UserTC);
  ...
  describe('Resolver.args', () => {
    it('should have `filter` optional arg', () => {
      const resolver = findOneLean(UserModel, UserTC);
      expect(resolver.getArgTypeName('filter')).toBe('FilterFindOneLeanUserInput');
    });
  ...
  describe('Resolver.resolve():Promise', () => {
    it('should be fulfilled promise', async () => {
      const result = findOneLean(UserModel, UserTC).resolve({});
      await expect(result).resolves.toBeDefined();
    });

    it('should return one document if args is empty', async () => {
      const result = await findOneLean(UserModel, UserTC).resolve({ args: {} });
    ...
    it('should skip records', async () => {
      const result = await findOneLean(UserModel, UserTC).resolve({ args: { skip: 2000 } });
      expect(result).toBeNull();
    });

    it('should sort records', async () => {
      const result1 = await findOneLean(UserModel, UserTC).resolve({
        args: { sort: { _id: 1 } },
      });

      const result2 = await findOneLean(UserModel, UserTC).resolve({
        args: { sort: { _id: -1 } },
      });
  ...
  describe('Resolver.getType()', () => {
    it('should return model type', () => {
      const outputType = findOneLean(UserModel, UserTC).getType();
