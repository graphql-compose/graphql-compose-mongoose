...
describe('findManyLean() ->', () => {
  ...
  it('should return Resolver object', () => {
    const resolver = findManyLean(UserModel, UserTC);
    expect(resolver).toBeInstanceOf(Resolver);
  });

  it('should have `filter` optional arg', () => {
    const resolver = findManyLean(UserModel, UserTC);
    expect(resolver.getArgTypeName('filter')).toBe('FilterFindManyLeanUserInput');
  });
  ...
  describe('Resolver.resolve():Promise', () => {
    it('should be fulfilled Promise', async () => {
      const result = findManyLean(UserModel, UserTC).resolve({});
      await expect(result).resolves.toBeDefined();
    });

    it('should return array of documents if args is empty', async () => {
      const result = await findManyLean(UserModel, UserTC).resolve({});
    ...
    it('should limit records', async () => {
      const result = await findManyLean(UserModel, UserTC).resolve({ args: { limit: 1 } });
    ...
    it('should skip records', async () => {
      const result = await findManyLean(UserModel, UserTC).resolve({ args: { skip: 1000 } });
    ...
    it('should sort records', async () => {
      const result1 = await findManyLean(UserModel, UserTC).resolve({
        args: { sort: { _id: 1 } },
      });

      const result2 = await findManyLean(UserModel, UserTC).resolve({
        args: { sort: { _id: -1 } },
      });
    ...
    it('should return mongoose documents', async () => {
      const result = await findManyLean(UserModel, UserTC).resolve({ args: { limit: 2 } });
