...
export function mergeCustomizationOptions<TContext>(
...
): ComposeWithMongooseOpts<TContext> | undefined {
  if (!baseCOptions) {
    return childCOptions;
  }

  const mergedOptions: ComposeWithMongooseOpts<TContext> = childCOptions || {};
  ...
  if (baseCOptions.inputType && baseCOptions.inputType.fields) {
    if (mergedOptions.inputType) {
      mergedOptions.inputType.fields = mergeFieldMaps(
        baseCOptions.inputType.fields,
        mergedOptions.inputType.fields
      );
    } else {
